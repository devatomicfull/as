...
Usa o CameraController: Fornece a maioria das funcionalidades principais do Camerax em uma única classe.
- Requer codigo de configuração
- Processa automaticamente a inicialização da câmera
- Gerenciamento de CASOS DE USO : 
    O QUE SERIA "caso de uso (Use Case)" no Camerax?
    No contexto do CameraX, um caso de uso é um tipo especifico de tarefa que você quer que a câmera faça, por exemplo:
